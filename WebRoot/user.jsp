...
<html>
  ...
  <% User viewedUser = (User)request.getAttribute("vieweduser");
     ...
     if(null != viewedUser) {
  %>
  <h6><%=viewedUser.getUsername() %></h6>， <%=viewedUser.getTagline() %>
  <br>
  简介：<br><p><%=viewedUser.getBio() %>
  
  <div style="cursor:hand" onclick="return follow(<%=viewedUser.getId()%>, <%=user.getId()%>, this)"><font color="red" size="16" id="followorno">关注我</font></div>
